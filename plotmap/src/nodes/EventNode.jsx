...
export default function EventNode({ data, selected }) {
  ...
  // ── Ghost: parent-layer context node (relLayer === -1) ───────────────────
  if (relLayer === -1) {
    return (
      <div
        className={`node node--ghost node--event node--${shape} ${selected ? 'node--selected' : ''}`}
        style={nodeStyle}
      ...
      >
        ...
        <div className="node__badge">↑ Event · L{data.layer ?? 0}</div>
        <div className="node__title">{data.title || 'Untitled Event'}</div>
        {data.description && <div className="node__desc">{data.description}</div>}
        {data.time && <div className="node__meta">⏱ {data.time}</div>}
  ...
  if (relLayer === 2) {
    ...
    return (
      <div className="node--micro node--event" style={dotStyle} title={data.title}>
        <Handle type="target" id="top-target"    position={Position.Top}    style={{ opacity: 0 }} />
  ...
  // ── Mini: compact pill with title only ────────────────────────────────────
  if (relLayer === 1) {
    return (
      <div
        className={`node node--mini node--event node--${shape} ${selected ? 'node--selected' : ''}`}
        style={nodeStyle}
      ...
      >
        ...
        <Handle type="target" id="left-target"   position={Position.Left}   style={{ opacity: 0 }} />
        <Handle type="source" id="left-source"   position={Position.Left} />
        <span className="node__mini-title">{data.title || 'Event'}</span>
        <Handle type="target" id="right-target"  position={Position.Right}  style={{ opacity: 0 }} />
        <Handle type="source" id="right-source"  position={Position.Right} />
        <Handle type="target" id="bottom-target" position={Position.Bottom} style={{ opacity: 0 }} />
        <Handle type="source" id="bottom-source" position={Position.Bottom} />
  ...
  return (
    <div
      className={`node node--event node--${shape} ${selected ? 'node--selected' : ''}`}
      style={nodeStyle}
    >
      <Handle type="target" id="top-target"    position={Position.Top}    style={{ opacity: 0 }} />
      <Handle type="source" id="top-source"    position={Position.Top} />
      ...
      <div className="node__badge">Event</div>
      <div className="node__title">{data.title || 'Untitled Event'}</div>
